...
export interface OfficialHerbInfo {
  herbName: string;
  ...
  link?: string;
  isNaverData?: boolean;
  isLocalDbData?: boolean;
  isPublicApiData?: boolean;
  originSpecies?: string;      // 기원종 (한국한의약진흥원 DB)
  usedPart?: string;           // 약용부위 (한국한의약진흥원 DB)
  ecologyDescription?: string; // 생태설명 (산림청 생태 DB)
  distributionRegion?: string; // 분포/자생지 (산림청 생태 DB)
  dosageMethod?: string;       // 복용방법 (산림청 생태 DB)
}

export interface SanYakBoGamInfo {
  herbName: string;
  ...
  oneLineImpression?: string;
  detailedInfo?: string;
  officialHerbData?: OfficialHerbInfo;
}

export type MainTab = 'home' | 'realEstate' | 'hotPlace' | 'sanYakBoGam' | 'generator' | 'keywordFighter' | 'shortcuts';
