...
const NaverNewsResults: React.FC<NaverNewsResultsProps> = ({ data, onGenerateStrategy, strategyLoading, strategy, onTopicSelect }) => {
    ...
    return (
        <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border border-slate-200 animate-fade-in space-y-4">
            <h3 className="flex items-center text-lg font-bold text-emerald-700">
            ...
            <div className="space-y-4">
                {data.map((item) => (
                    <div key={item.id} className="bg-slate-50 p-4 rounded-lg border border-slate-200 hover:border-teal-400 transition-colors shadow-sm">
                        <a href={item.url} target="_blank" rel="noopener noreferrer" className="block">
                             <p className="text-xs text-slate-500 mb-1">{formatDate(item.pubDate)}</p>
                             <h4 className="font-bold text-teal-700 hover:underline mb-2">{item.title}</h4>
                    ...
                    <div className="space-y-4">
                        {strategy.map((idea) => (
                            <div 
                                ...
                                className="bg-emerald-50/50 p-4 rounded-lg border border-emerald-200 hover:border-emerald-400 hover:shadow-md transition-all duration-200 cursor-pointer group"
                            >
                                <h4 className="font-bold text-emerald-800 group-hover:underline">{idea.title}</h4>
                                <div className="flex flex-wrap gap-2 my-2">
                                    {idea.keywords.map(kw => (
                                        <span key={kw} className="bg-emerald-100 text-emerald-800 text-xs font-medium px-2.5 py-1 rounded-full">{kw}</span>
                                    ))}
                                </div>
                                <p className="text-sm text-slate-700 leading-relaxed">{idea.strategy}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {!strategy && (
                <div className="mt-6 pt-4 border-t border-slate-200 text-center">
                    <button
                        ...
                        className="bg-emerald-600 text-white font-bold py-3 px-6 rounded-md hover:bg-emerald-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed transition duration-300 flex items-center justify-center w-full sm:w-auto mx-auto shadow-sm"
                    >
                        {strategyLoading ? (
                            <>
